"""Higenstock method for 3D elliptic grid generation.

SOR iteration with orthogonality-based source term control.
Matches C higen_gene in elliptic.c.
"""
...
def higen_gene(lib, gdinfo, x3d, y3d, z3d, bdry, mympi, cfgs):
    """Run Higenstock elliptic grid generation.

    Matches C higen_gene (elliptic.c lines 1157-1310):
      Before loop: dist_cal → set_src_higen → interp_source → copy x3d to x3d_tmp
      Loop: update_SOR → exchange_ghost(x3d_tmp) → residual → allreduce → swap
            → set_src_higen → interp_source → check convergence
    """
    nx = gdinfo['nx']
    ny = gdinfo['ny']
    nz = gdinfo['nz']
    ...
    # Global and local boundary source terms
    src_global = {}
    src_local = {}
    ...
    # Distance arrays for Higenstock (1D, matching C dist_cal)
    dx1 = np.zeros(nz * ny, dtype=np.float32)
    dx2 = np.zeros(nz * ny, dtype=np.float32)
    dy1 = np.zeros(nz * nx, dtype=np.float32)
    dy2 = np.zeros(nz * nx, dtype=np.float32)
    dz1 = np.zeros(ny * nx, dtype=np.float32)
    dz2 = np.zeros(ny * nx, dtype=np.float32)

    neighid_c = np.ascontiguousarray(mympi.neighid)

    # Calculate initial distances — called ONCE before loop (matching C line 1194)
    lib.dist_cal_c(x3d_c, y3d_c, z3d_c, nx, ny, nz,
    ...
    _set_src_higen_c(lib, gdinfo, x3d_c, y3d_c, z3d_c,
    ...
    # Copy current coordinates to tmp (matching C lines 1199-1210)
    x3d_tmp_c[:] = x3d_c
    y3d_tmp_c[:] = y3d_c
    z3d_tmp_c[:] = z3d_c
    ...
    for it in range(max_iter):
        ...
        # Recompute source terms (matching C lines 1302-1303)
        P[:] = 0; Q[:] = 0; R[:] = 0
        _set_src_higen_c(lib, gdinfo, x3d_c, y3d_c, z3d_c,
                         P, Q, R, src_global, src_local,
                         dx1, dx2, dy1, dy2, dz1, dz2,
        ...
        if (global_max[0] < iter_err and
            ...
            if mympi.is_root():
                print(f"Higenstock method converged at iteration {it+1}")
            break
    else:
        if mympi.is_root():
            print(f"Higenstock method reached max iterations ({max_iter})")
...
def _set_src_higen_c(lib, gdinfo, x3d_c, y3d_c, z3d_c,
                     P, Q, R, src_global, src_local,
                     dx1, dx2, dy1, dy2, dz1, dz2,
                     neighid_c, mympi):
    """Call set_src_higen_c to compute boundary source terms."""
    nx = gdinfo['nx']
    ny = gdinfo['ny']
    nz = gdinfo['nz']
    ni = gdinfo['ni']
...
def _interp_source(lib, gdinfo, P, Q, R, src_global, coef_arr, n_coef):
    """Call interp_inner_source_c to interpolate source terms."""
    nx = gdinfo['nx']
    ny = gdinfo['ny']
    nz = gdinfo['nz']
    gni1 = gdinfo['gni1']
